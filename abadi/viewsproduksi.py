...
def add_sppb(request):
    ...
    if request.method == "GET":
        return render(request, "produksi/add_sppb.html",{'data':dataartikel, })
...
def detail_sppb(request,id):
    ...
    if request.method == "GET":
        ...
        return render(request,'produksi/detail_sppb.html',{'dataartikel':dataartikel,'data':datadetailspk,'datasppb':datasppb,'datadetail':datadetailsppb, 'tanggal':tanggal})
...
def view_mutasi(request):
    dataproduksi = models.TransaksiProduksi.objects.filter(Jenis="Mutasi").order_by("-Tanggal","KodeArtikel")

    return render(
        request, "produksi/view_mutasi.html", {"dataproduksi": dataproduksi}
    )


def view_produksi(request):
    dataproduksi = models.TransaksiProduksi.objects.filter(Jenis="Produksi").order_by("-Tanggal","KodeArtikel")
...
def add_mutasi(request):
    if request.method == "GET":
        ...
        return render(
            ...
            {"kode_artikel": data_artikel, "nama_lokasi": data_lokasi, "data_spk":data_spk},
        )
    ...
    if request.method == "POST":
        ...
        for kode, lokasi, jumlah, keterangan, detail_spk in zip(listkode_artikel,listlokasi,listjumlah,listketerangan,listdetail_spk):
...
def add_produksi(request):
    ...
    if request.method == "POST":
        ...
        for kode,  jumlah, keterangan, detail_spk in zip(listkode_artikel,listjumlah,listketerangan,listdetail_spk):
            artikelref = models.Artikel.objects.get(KodeArtikel=kode)
...
def update_produksi(request, id):
    ...
    try:
        data_detailspk = models.DetailSPK.objects.filter(NoSPK=produksiobj.DetailSPK.NoSPK.id)
    except:
        data_detailspk = None
...
def view_gudang(request):
    datagudang = models.TransaksiGudang.objects.filter(jumlah__gt=0).order_by("-tanggal","KodeProduk")

    return render(request, "produksi/view_gudang.html", {"datagudang": datagudang})


def view_gudangretur(request):
    datagudang = models.TransaksiGudang.objects.filter(jumlah__lt=0).order_by("-tanggal","KodeProduk")
...
def add_gudang(request):
    if request.method == "GET":
        ...
        return render(
            ...
            {"kode_produk": data_produk, "nama_lokasi": data_lokasi, "data_spk":data_spk, "listproduk":listproduk},
        )
    ...
    if request.method == "POST":
        ...
        for produk, lokasi, jumlah, keterangan, detail in zip(listkode,listlokasi,listjumlah,listketerangan,listdetail):
...
def add_gudangretur(request):
    ...
    if request.method == "POST":
        ...
        for produk, lokasi, jumlah, keterangan in zip(listproduk,listlokasi,listjumlah,listketerangan):
...
def update_gudang(request, id):
    ...
    data_spk = models.SPK.objects.all()
    try:
        data_detailspk = models.DetailSPK.objects.filter(NoSPK=gudangobj.DetailSPK.NoSPK.id)
    except:
        data_detailspk = None
