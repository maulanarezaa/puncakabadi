...
def detail_spk(request, id):
    ...
    if request.method == "GET":
        ...
        existspk = models.SPK.objects.filter(NoSPK=nomor_spk).exists()
        if existspk:
            messages.error(request, "Nomor SPK sudah ada")
            ...
            if valid_artikel_list or valid_display_list:
                ...
                for artikel, jumlah, confirmationorder in zip(artikel_list, jumlah_list,confirmationorderartikel):
                    ...
                    models.transactionlog(
                        ...
                        waktu=datetime.now(),
                        jenis="Create",
                        pesan=f"Detail Surat Perintah Pengiriman Barang. Nomor SPPB : {no_sppb.NoSPPB} Kode Artikel : {kode_artikel.KodeArtikel} Jumlah : {jumlah} CO : {datadetailspk.IDCO.NoCO}",
                    ).save()
                ...
                for display,jumlah,confirmationorder in zip(displaylist,jumlahdisplay,confirmationorderdisplay):
                    ...
                    models.transactionlog(
                        ...
                        waktu=datetime.now(),
                        jenis="Create",
                        pesan=f"Detail Surat Perintah Pengiriman Barang. Nomor SPPB : {no_sppb.NoSPPB} Kode Display : {detailspkdisplayobj.KodeDisplay} Jumlah : {jumlah} CO : {datadetailspkdisplay.IDCO.NoCO}",
                    ).save()
...
def detail_sppb(request, id):
    ...
    if request.method == "GET":
        ...
        existsppb = models.SPPB.objects.filter(NoSPPB=nomor_sppb).exists()
        if existsppb:
            messages.error(request, "Nomor SPPB sudah ada")
        ...
        if display_baru:
            print(display_baru)
            print(jumlahdisplay_baru)
            print(purchaseorder_displaybaru)
            for display_id, jumlah, confirmationorder in zip(display_baru,jumlahdisplay_baru,purchaseorder_displaybaru):
                ...
                models.transactionlog(
                    ...
                    waktu=datetime.now(),
                    jenis="Create",
                    pesan=f"Detail Surat Perintah Pengiriman Barang. Nomor SPPB : {no_sppb.NoSPPB} Kode Artikel : {kode_display.KodeDisplay} Jumlah : {jumlah}",
                ).save()
