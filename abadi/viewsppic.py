...
def getpenyusunartikelpertanggal(tanggal,kodeartikel):
    ...
    return datapenyusun
def getstokfg(lastdays, stopindex,awaltahun,hargapurchasing=None):
    waktuawal = time.time()
    ...
    for index, hari in enumerate(lastdays[:stopindex]):
        ...
        # if index == 1:
        #     print('selisihwaktu : ',akhir1loop-awal)
        #     print(asd)
    return data
...
def detaillaporanstokfg(request):
    if len(request.GET) > 0:
        ...
        tes = getstokfg(last_days,index,awaltahun,hargaakhirbulanperproduk)
...
def getstokbahanproduksi(last_days, stopindex, awaltahun):
    ...
    stokawalbahanproduksi[0] = {"data": bahanbaku, "total": totalbiayasaldoawal_wip}
...
def exportlaporanbulananexcel(request):
    ...
    if datacache :
        ...
        stokfg = getstokfg(last_days,index,awaltahun,hargapurchasing)
...
def exportlaporanbulananexcelkeseluruhan(request):
    ...
    with pd.ExcelWriter(buffer, engine="openpyxl") as writer:
        ...
        if len(listdatabahanbakukeluar) != 0 or len(listdatamodelkeluar)!=0 or len(listdatadisplaykeluar) !=0 or len(listtransaksigolongand)!=0 or len(listtransaksilainlain) !=0 :
            maxcolumn  = 0
            maxrow = 0
            maxcolprevdf = 0
            # ARTIKEL KELUAR (SPPB)
